          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <select value={sortMode} onChange={event => setSortMode(event.target.value as "soonest" | "popular")} className="h-10 appearance-none rounded-lg border border-[var(--brand-purple)]/25 bg-[var(--surface)] pl-9 pr-9 text-sm text-[var(--foreground)] outline-none">
                  <option value="soonest">Soonest first</option>
                  <option value="popular">Most popular</option>
                </select>
                <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
              </div>
              <Link href="/events/all" className="inline-flex h-10 items-center gap-2 rounded-xl bg-[var(--brand-purple)] px-4 text-sm font-bold text-white transition hover:opacity-90">See all <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </div>
    ...
    <Link href={`/events/${event.id}`} className="group flex h-full flex-col overflow-hidden rounded-xl border-[0.5px] border-[var(--brand-purple)]/35 bg-[var(--surface)] transition duration-300 hover:-translate-y-1 hover:border-[var(--brand-purple)]">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-[var(--brand-purple)]/15 pt-4 text-xs font-semibold text-[#848d92]">
          ...
          <span className="text-[var(--brand-purple)]">View event</span>
        </div>
      </div>
    </Link>
  );
