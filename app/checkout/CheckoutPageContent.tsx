...
const INPUT =
  "w-full bg-gray-800 text-gray-300 rounded-xl border border-gray-700 px-4 py-3 text-sm focus:ring-2 focus:ring-purple-500 outline-none transition placeholder-gray-600";
...
export default function CheckoutPageContent() {
  ...
  useEffect(() => {
    try {
      if (!ticketsJSON) {
        setParseError("No ticket data. Please go back.");
        return;
      }
      ...
      setParseError(
        "Could not load ticket data. Please go back and try again.",
      );
    }
  ...
  function startPolling(ref: string) {
    ...
    pollRef.current = setInterval(async () => {
      ...
      if (elapsed >= 180) {
        ...
        setStatus("error");
        setErrorMsg(
          "Payment timed out. If you completed the M-Pesa prompt, please contact support with your reference.",
        );
        return;
      }
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      const res = await fetch(
        `/api/promos/validate?eventId=${eventId}&code=${promoInput.trim().toUpperCase()}`,
      );
      const data = await res.json();
      if (!res.ok) {
  ...
  function calcDiscount(sub: number, discount: string): number {
    if (discount.trim().endsWith("%"))
      return sub * (parseFloat(discount) / 100);
    return Math.min(sub, parseFloat(discount.replace(/[^0-9.]/g, "")) || 0);
  }

  const discountAmount = promoApplied
    ? calcDiscount(subtotal, promoApplied.discount)
    : 0;
  const total = Math.max(0, subtotal - discountAmount);
  const isRsvp =
    cartItems.length > 0 &&
    cartItems.every(
      (t) => t.price === "Free" || t.price === "0" || t.ticketType === "RSVP",
    );
  ...
  const handleSubmit = async () => {
    ...
    if (!/^\+?\d{9,15}$/.test(phone.replace(/\s/g, ""))) {
      setErrorMsg("Please enter a valid phone number (e.g. +254712345678).");
      return;
    }
    if (!eventId) {
      setErrorMsg("Event ID is missing. Please go back.");
      return;
    }
        ...
        <div className="w-full max-w-md flex flex-col items-center gap-6 text-center">
          ...
          <div className="w-full bg-gray-900 border border-gray-700 rounded-2xl p-5">
            <p className="text-gray-500 text-xs mb-3">
              ...
              {".".repeat((Math.floor(pollSeconds / 3) % 3) + 1)}
            </p>
            <div className="w-full bg-gray-800 rounded-full h-1.5">
              <div
                className="bg-purple-500 h-1.5 rounded-full transition-all duration-1000"
                style={{
                  width: `${Math.min((pollSeconds / 180) * 100, 100)}%`,
                }}
              />
            </div>
            <p className="text-gray-700 text-xs mt-2">
              {180 - pollSeconds}s remaining
            </p>
          </div>
          <p className="text-gray-600 text-xs">
            Don't close this page. You'll be redirected automatically once
            confirmed.
          </p>
          <button
            onClick={() => {
        ...
        <div className="w-full max-w-md flex flex-col items-center gap-6 text-center">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">
              ...
              <strong className="text-gray-300">{email}</strong> and SMS to{" "}
          ...
          {cartItems.map((item, i) => (
            <div key={i} className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600 text-xs">
                  {isRsvp ? "" : `${item.quantity} × KES ${item.price}`}
                </p>
              </div>
              {!isRsvp && (
              ...
              {promoApplied && (
                <div className="flex justify-between text-sm">
                  <span className="text-green-400 flex items-center gap-1">
                    <Tag className="w-3.5 h-3.5" /> {promoApplied.code} (
        ...
        {promoApplied && (
          <div className="flex items-center justify-between bg-green-900/20 border border-green-700/40 rounded-xl px-4 py-3">
            <span className="text-green-400 text-sm font-semibold flex items-center gap-2">
              <CheckCircle className="w-4 h-4" /> {promoApplied.code} —{" "}
              {promoApplied.discount} off
            </span>
            <button
        ...
        {!isRsvp && (
          <div className="flex items-start gap-3 bg-blue-900/20 border border-blue-700/30 rounded-xl px-4 py-3">
            ...
            <p className="text-blue-300 text-sm">
              After clicking pay, you'll receive an M-Pesa prompt on{" "}
              <strong>{phone || "your phone"}</strong>. Enter your PIN to
              complete.
            </p>
          </div>
        )}
