...
export default function CheckoutPageContent() {
  ...
  // ← read the referrer platform that was set by the short link redirect
  ...
  const handleSubmit = async () => {
    ...
    if (!agreedToTerms)    { setErrorMsg("Please accept the Terms & Conditions to continue."); return; }
        ...
        <div className="w-full max-w-md flex flex-col items-center gap-6 text-center">
          ...
          <div className="w-full bg-gray-900 border border-gray-700 rounded-2xl p-5">
            <p className="text-gray-500 text-xs mb-3">
              Waiting for confirmation{".".repeat((Math.floor(pollSeconds / 3) % 3) + 1)}
            </p>
          ...
          <p className="text-gray-600 text-xs">Don't close this page. You'll be redirected automatically once confirmed.</p>
          <button
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-16">
        <div className="w-full max-w-md flex flex-col items-center gap-6 text-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-100 mb-2">{isRsvp ? "You're in!" : "Order confirmed!"}</h1>
            <p className="text-gray-400 text-sm">
              Confirmation sent to <strong className="text-gray-300">{email}</strong> and SMS to{" "}
      ...
      <div className="w-full max-w-lg flex flex-col gap-5">
        ...
        {!isRsvp && (
          <div className="flex items-start gap-3 bg-blue-900/20 border border-blue-700/30 rounded-xl px-4 py-3">
            ...
            <p className="text-blue-300 text-sm">
              ...
              <strong>{phone || "your phone"}</strong>. Enter your PIN to complete.
            </p>
          </div>
        )}
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-gray-400 leading-snug">
            ...
            including the{" "}
            <a href="/terms#payments" target="_blank" rel="noopener noreferrer"
              className="font-semibold text-purple-400 hover:text-purple-300 underline underline-offset-2 transition">
        ...
        <button onClick={handleSubmit}
          ...
          {status === "loading"
            ...
            : isRsvp ? "Confirm RSVP →" : `Pay KES ${total.toLocaleString()} via M-Pesa →`}
        </button>
