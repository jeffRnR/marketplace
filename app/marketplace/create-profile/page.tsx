  ...
  return (
    <div>
      ...
      {preview ? (
        ...
        >
          {uploading ? (
            <><Loader2 className="w-6 h-6 text-purple-500 animate-spin" /><span className="text-[var(--muted)] text-xs">Uploading…</span></>
          ) : (
          ...
          {STEPS.map((s, i) => (
            <React.Fragment key={s}>
              <div className={`flex items-center gap-2 text-sm font-semibold transition-colors ${
                i === step ? "text-purple-500" : i < step ? "text-green-400" : "text-[var(--muted)]"
              }`}>
