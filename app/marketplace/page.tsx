...
import {
  ...
  ChevronRight, BadgeCheck, Store, X,
...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(i => (
        <Star key={i} className={`w-3.5 h-3.5 ${i <= Math.round(rating) ? "fill-amber-400 text-amber-400" : "text-gray-600"}`} />
      ))}
    </div>
  );
}
...
function PriceBadge({ listing }: { listing: Listing }) {
  if (listing.priceType === "negotiable") return <span className="text-xs text-purple-400 font-semibold">Negotiable</span>;
  if (listing.priceType === "free")       return <span className="text-xs text-green-400 font-semibold">Free</span>;
  if (!listing.price) return null;
  const suffix = listing.priceType === "hourly" ? "/hr" : listing.priceType === "daily" ? "/day" : "";
  return <span className="text-xs text-amber-400 font-semibold">{listing.currency} {listing.price.toLocaleString()}{suffix}</span>;
}
      ...
      <div className="relative bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden hover:border-purple-700/60 transition-all duration-300 hover:shadow-xl hover:shadow-purple-900/20 hover:-translate-y-0.5">
        ...
        <div className="relative h-36 bg-gradient-to-br from-gray-800 to-gray-900 overflow-hidden">
          {profile.coverImage
            ...
            : <div className="absolute inset-0 flex items-center justify-center opacity-10"><Icon className="w-24 h-24" /></div>
          }
          <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/20 to-transparent" />

          {profile.isVerified && (
            <div className="absolute top-3 right-3 flex items-center gap-1 bg-purple-600/90 text-white text-xs px-2 py-1 rounded-full font-semibold">
        ...
        <div className="px-4 pt-2 pb-4">
          ...
          {profile.tagline && (
            <p className="text-gray-400 text-xs mb-2 leading-snug line-clamp-1">{profile.tagline}</p>
          )}
          ...
          {topListing && (
            <div className="bg-gray-800/60 rounded-xl p-2.5 mb-3 border border-gray-700/50">
              <div className="flex justify-between items-start gap-2">
                <p className="text-gray-300 text-xs font-medium truncate">{topListing.title}</p>
          ...
          <div className="flex items-center justify-between">
            <span className="text-gray-600 text-xs">
              {profile._count?.listings ?? profile.listings.length} listing{(profile._count?.listings ?? profile.listings.length) !== 1 ? "s" : ""}
            </span>
      ...
      <div className="border-b border-gray-800">
        <div className="max-w-6xl mx-auto px-4 py-14">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              <p className="text-gray-500 text-sm font-semibold uppercase tracking-widest mb-3">Event Ecosystem</p>
            ...
            <div className="flex flex-col gap-3">
              {session?.user ? (
                ...
                <p className="text-gray-500 text-sm">Sign in to list your services</p>
              )}

              <div className="flex gap-3 text-center">
                <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-2 flex-1">
                  <p className="text-gray-100 font-bold text-lg">{profiles.length}</p>
                  <p className="text-gray-500 text-xs">Vendors</p>
                </div>
                <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-2 flex-1">
                  <p className="text-gray-100 font-bold text-lg">{CATEGORIES.length - 1}</p>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-3 mb-6">
          ...
          >
            ...
            {category && <span className="bg-white/20 rounded-full w-4 h-4 text-xs flex items-center justify-center">1</span>}
          </button>
        </div>
        ...
        {(category || debouncedSearch) && (
          <div className="flex items-center gap-2 mb-6 flex-wrap">
            ...
            <span className="text-gray-600 text-xs">{profiles.length} result{profiles.length !== 1 ? "s" : ""}</span>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-gray-400 font-semibold text-lg mb-2">No vendors found</h3>
