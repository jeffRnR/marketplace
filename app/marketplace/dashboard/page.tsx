...
// Marketplace vendor dashboard — listings, inquiries, settings.
// Applications tab removed: vendors are contacted directly via inquiries.
...
function ListingForm({ initial, profileCategory, onSave, onCancel, saving }: {
  ...
  const [form, setForm] = useState({
    ...
    category: initial?.category ?? profileCategory, priceType: initial?.priceType ?? "fixed",
    price: initial?.price?.toString() ?? "", images: initial?.images?.join(", ") ?? "",
    ...
    <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-5 space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input value={form.title} onChange={e => set("title", e.target.value)} placeholder="e.g. Wedding Photography Package"
      ...
      <div className="flex gap-3 pt-2">
        <button onClick={() => onSave({ ...form, price: form.price ? Number(form.price) : null,
        ...
        <button onClick={onCancel} className="px-4 py-2.5 border border-gray-700 text-gray-400 rounded-lg text-sm hover:border-gray-600 transition">Cancel</button>
      </div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      if (data?.id) {
        setProfile(data);
        setSettingsForm({ businessName: data.businessName, tagline: data.tagline??"", description: data.description,
          location: data.location, phone: data.phone, email: data.email, website: data.website??"",
          instagram: data.instagram??"", coverImage: data.coverImage??"", logoImage: data.logoImage??"", isActive: data.isActive });
      } else { router.push("/marketplace/create-profile"); }
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        {tab === "listings" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              {!showAdd && <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white font-semibold px-4 py-2 rounded-xl text-sm transition"><Plus className="w-4 h-4"/>Add Listing</button>}
            </div>
            ...
            {profile.listings.length === 0 && !showAdd ? (
              ...
              <div key={listing.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                {editId === listing.id ? (
                  <div className="p-4"><ListingForm initial={listing} profileCategory={profile.category} saving={saving} onSave={handleUpdateListing} onCancel={() => setEditId(null)} /></div>
                ) : (
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full border shrink-0 ${listing.isActive?"bg-green-900/30 border-green-700/30 text-green-400":"bg-gray-800 border-gray-700 text-gray-500"}`}>{listing.isActive?"Active":"Paused"}</span>
                        </div>
                        <p className="text-gray-500 text-sm line-clamp-2 mb-2">{listing.description}</p>
                        <div className="flex items-center gap-3 text-xs">
                          <span className="text-purple-400 font-semibold">{listing.priceType==="negotiable"?"Negotiable":listing.priceType==="free"?"Free":listing.price?`KES ${listing.price.toLocaleString()}${listing.priceType==="hourly"?"/hr":listing.priceType==="daily"?"/day":""}`:"—"}</span>
        ...
        {tab === "settings" && settingsForm && (
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
            ...
            {(["businessName","tagline","location","phone","email","website","instagram","coverImage","logoImage"] as const).map(key => (
              <div key={key}>
                <label className="block text-gray-400 text-sm mb-1.5 capitalize">{key.replace(/([A-Z])/g," $1").trim()}{key==="businessName"&&<span className="text-red-400"> *</span>}</label>
            ...
            <div>
              ...
              <textarea value={settingsForm.description} onChange={e => setSettingsForm((f:any) => ({...f,description:e.target.value}))} rows={4}
