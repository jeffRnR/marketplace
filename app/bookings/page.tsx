...
function BookingCard({
  ...
  return (
    <div className="bg-gray-900 border border-gray-400/20 rounded-2xl overflow-hidden">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span className="text-[var(--muted)] text-xs">{relTime(booking.createdAt)}</span>
            </div>
            <p className="text-[var(--foreground)] font-bold text-base leading-snug">{booking.listing.title}</p>
            <p className="text-[var(--muted)] text-sm mt-0.5">
              {isVendor ? `Requested by ${other}` : `From ${other}`}
            </p>
          </div>
          <div className="text-right shrink-0">
            <p className="text-[var(--brand-green)] font-black text-lg">
              KES {booking.totalAmount.toLocaleString()}
            </p>
            {booking.eventDate && (
              <p className="text-[var(--muted)] text-xs flex items-center gap-1 justify-end mt-0.5">
        ...
        <button
          ...
          className="flex items-center gap-1 text-[var(--muted)] hover:text-[var(--foreground)] text-xs mt-3 transition"
        >
        ...
        {expanded && (
          <div className="mt-3 bg-white/2 border border-gray-400/20 rounded-xl p-3 space-y-1.5 text-xs text-[var(--muted)]">
            <p>Qty: <span className="text-[var(--foreground)]">{booking.quantity}</span></p>
            {booking.notes && <p>Notes: <span className="text-[var(--foreground)]">{booking.notes}</span></p>}
            <p>Payment: <span className="text-[var(--foreground)]">{booking.paymentMethod === "mpesa" ? "M-Pesa" : "Offline"}</span></p>
      ...
      {/* Error */}
      {error && (
        <div className="mx-4 mb-3 flex items-center gap-2 bg-red-900/20 border border-red-800/50 rounded-xl px-3 py-2 text-red-400 text-xs">
      ...
      {isVendor && booking.status === "approved" && booking.paymentMethod === "offline" && (
        <div className="px-4 pb-4">
          <button
            onClick={() => doAction("confirm_offline")}
            disabled={submitting}
      ...
      {!isVendor && booking.status === "approved" && booking.paymentMethod === "mpesa" && !waitingPay && (
        <div className="px-4 pb-4 flex flex-col gap-2">
          <input
            ...
            className="w-full bg-white/5 border border-gray-400/50 rounded-xl px-4 py-2.5 text-[var(--foreground)] text-sm placeholder:text-[var(--muted)] outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-600/30 transition"
          />
          <button
            onClick={() => doAction("pay", { phone })}
      ...
      {!isVendor && ["pending", "approved"].includes(booking.status) && (
        <div className="px-4 pb-4">
          <button
            ...
            disabled={submitting}
            className="w-full text-[var(--muted)] hover:text-red-400 text-xs transition py-1"
          >
            Cancel booking
          </button>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="p-2 text-[var(--muted)] hover:text-[var(--foreground)] transition"
        ...
        {/* Filter tabs */}
        <div className="flex gap-1 bg-gray-900 border border-gray-400/20 rounded-xl p-1 mb-6">
          {FILTERS.map(f => (
            <button
              key={f.id}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-gray-900 border border-gray-400/20 rounded-2xl">
            <Package className="w-12 h-12 text-[var(--muted)] mx-auto mb-4 opacity-40" />
            <p className="text-[var(--foreground)] font-semibold">No bookings here</p>
            <p className="text-[var(--muted)] text-sm mt-1">
              {filter === "all"
                ? "Bookings from the marketplace will appear here."
                : `No ${filter} bookings.`}
            </p>
          </div>
        ) : (
