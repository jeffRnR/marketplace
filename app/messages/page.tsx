...
// Full messaging interface — conversation list + chat panel + booking widget.
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        {!isVendor && listing && bookings.every(b => ["rejected","cancelled"].includes(b.status)) && (
        ...
        {!isVendor && listing && bookings.length === 0 && (
          <button onClick={() => setShowForm(v => !v)}
            className="flex items-center gap-1 text-xs text-purple-400 hover:text-purple-300 font-semibold transition">
            <ShoppingCart className="w-3.5 h-3.5" />
            {showForm ? "Cancel" : "Request booking"}
      ...
      {bookings.map(booking => (
        <div key={booking.id} className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex flex-col gap-3">
          ...
          <div className="text-xs text-gray-500 space-y-1">
            ...
            {booking.vendorNote && (
              <p className="text-amber-400">Vendor note: {booking.vendorNote}</p>
            )}
          </div>
          ...
          {!isVendor && booking.status === "approved" && booking.paymentMethod === "offline" && (
            <div className="flex items-center gap-2 bg-amber-900/20 border border-amber-700/30 rounded-xl px-3 py-2 text-amber-300 text-xs">
              ...
              Arrange payment directly with the vendor.
            </div>
          )}
...
export default function MessagesPage() {
  ...
  // Derive my user id from session
  useEffect(() => {
    if (!session?.user?.email) return;
    fetch("/api/messages/unread-count").then(() => {}); // warm up
  ...
  const allConvs   = [...(conversations?.asBuyer ?? []), ...(conversations?.asVendor ?? [])];
            ...
            {loadingConvs ? (
              ...
              <div className="text-center py-16 px-4">
                ...
                <p className="text-gray-600 text-xs mt-1">
                  {activeTab === "buyer"
                    ? "Message a vendor from the marketplace."
                    : "Buyers will message you from your profile."}
                </p>
              </div>
                      ...
                      <div key={msg.id}>
                        ...
                        <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[75%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-0.5`}>
                            {!isMe && (
                              <p className="text-gray-500 text-xs ml-1">{msg.sender.name ?? msg.sender.email}</p>
                            )}
                    ...
                    onAction={() => {
                      fetch(`/api/messages/${activeId}`)
                        ...
                        .then(d => {
                          ...
                          setConvDetail(d.conversation ?? null);
                        });
                    }}
                  />
                </div>
