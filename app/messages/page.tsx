...
// Unified messaging interface — bookings have moved to /bookings
...
function MessagesPageInner() {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (msg.senderId !== myId) {
          fetch(`/api/messages/${activeId}/read`, {
            method: "PATCH",
          }).catch(() => {});
        }
  ...
  // Combine vendor and buyer conversations into one platform
  const allConversations = [
    ...(conversations?.asBuyer ?? []),
    ...(conversations?.asVendor ?? []),
  ];

  // Remove duplicates in case the same conversation appears in both lists
  const displayedConvs = Array.from(
  ...
  const isVendorInActive = convDetail
    ? convDetail.vendorProfile?.userId === myId
    : false;

  function convName(conv: ConvSummary) {
    const isMyVendorConversation =
      conv.vendorProfile?.userId === myId;

    return isMyVendorConversation
      ? (conv.buyer?.name ?? conv.buyer?.email ?? "Buyer")
      : (conv.vendorProfile?.businessName ?? "Vendor");
  }

  function convAvatar(conv: ConvSummary) {
    const isMyVendorConversation =
      conv.vendorProfile?.userId === myId;

    return isMyVendorConversation
      ? null
      : (conv.vendorProfile?.logoImage ?? null);
  }

  const pendingBookings =
    convDetail?.bookings?.filter(
      (b) => b.status === "pending"
    ).length ?? 0;
  ...
  return (
    <div className="min-h-screen pt-0 flex flex-col">
      <div
        className="flex-1 flex overflow-hidden mx-auto w-full lg:w-[70%]"
        style={{ height: "calc(100vh - 96px)" }}
      >
        {/* ── Sidebar ── */}
        <div
          className={`w-full md:w-64 lg:w-72 border-r border-gray-600 flex flex-col shrink-0 ${
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingConvs ? (
              ...
              <div className="text-center py-16 px-4">
                ...
                <p className="text-gray-600 text-xs mt-1">
                  Your conversations with buyers and vendors will appear here.
                </p>
              </div>
            ) : (
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-600 shrink-0">
              <button
                ...
                className="md:hidden text-gray-500 hover:text-gray-300 transition"
              >
              ...
              {convDetail?.vendorProfile?.logoImage &&
                !isVendorInActive && (
                  <img
                    src={convDetail.vendorProfile.logoImage}
                    alt=""
              ...
              <div className="min-w-0 flex-1">
                <p className="text-gray-100 font-semibold text-sm truncate">
                  {isVendorInActive
                    ? (convDetail?.buyer?.name ??
                        convDetail?.buyer?.email ??
                        "Buyer")
                    : (convDetail?.vendorProfile?.businessName ??
                        "Vendor")}
                </p>
