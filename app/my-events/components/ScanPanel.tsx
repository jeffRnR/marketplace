...
function LogsDrawer({ station }: { station: Station }) {
    ...
    async (p = 1) => {
      ...
      try {
        const res = await fetch(
          `/api/scan/logs?stationId=${station.id}&page=${p}`,
        );
  ...
  const resultCounts = logs.reduce(
    (acc, l) => {
  ...
  return (
    <div className="border-t border-gray-800 bg-gray-950/60 p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-gray-500 text-xs font-semibold uppercase tracking-widest mr-1">
          Filter
        </span>
        {(
          [
        ...
        <button
          ...
          className="ml-auto p-1 text-gray-600 hover:text-gray-400 transition"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-1.5">
          {filtered.map((log) => {
            ...
            // Strip internal debug prefix from note for display
            const displayNote =
              log.note
                ?.replace(
                  /^IP: [^\s]+ \| sig: (verified|unsigned\/manual)\s*/,
                  "",
                )
                ?.replace(
                  /^Ticket not found or not for this event\.$/,
            ...
            return (
              <div
                key={log.id}
                className={`rounded-xl border px-3 py-2.5 ${meta.bg}`}
              >
                {/* Top row */}
                <div className="flex items-start gap-3">
                  {/* Result badge */}
                  <span
                    className={`text-xs font-bold shrink-0 mt-0.5 w-28 ${meta.color}`}
                  >
                    {meta.label}
                  </span>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-gray-600 text-xs">
                      {relTime(log.scannedAt)}
                    </p>
                  </div>
                </div>

                {/* Reason row — only for denies and warnings */}
                {displayNote && (isDeny || isWarning) && (
                  <p
                    className={`text-xs mt-1.5 pl-28 leading-snug ${
                      isDeny
                        ? "text-red-400/70"
                        : isWarning
                          ? "text-amber-400/70"
          ...
          >
            <div>
              ...
              <input
                ...
                placeholder="e.g. John at Gate 1"
...
function StationCard({
  ...
  async function revokeSession(sessionId: string) {
    ...
    await fetch(`/api/scan/sessions?sessionId=${sessionId}`, {
      method: "DELETE",
    });
    setRevoking(null);
    onRefresh();
  ...
  async function deleteStation() {
    ...
    await fetch(`/api/scan/stations?stationId=${station.id}`, {
      method: "DELETE",
    });
    onRefresh();
  }
  ...
  return (
    <>
      <div
        className={`bg-gray-900 border rounded-2xl overflow-hidden ${station.isActive ? "border-gray-800" : "border-gray-800 opacity-60"}`}
      >
        {/* Header */}
        <div className="flex items-center gap-3 p-4">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-black shrink-0 ${station.isFinal ? "bg-purple-600 text-white" : "bg-gray-700 text-gray-300"}`}
          >
            {station.order}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {station.isFinal && (
                <span className="text-xs bg-purple-900/40 border border-purple-700/40 text-purple-400 px-2 py-0.5 rounded-full">
                  Final
                </span>
              )}
              {!station.isActive && (
            ...
            <p className="text-gray-600 text-xs mt-0.5">
              {station._count.logs} scans · {activeSessions.length} active
              scanner{activeSessions.length !== 1 ? "s" : ""}
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              onClick={toggleFinal}
              className={`p-1.5 rounded-lg border transition ${station.isFinal ? "border-purple-700 text-purple-400" : "border-gray-700 text-gray-500 hover:text-gray-300"}`}
            >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-green-400 text-sm font-semibold mb-2">
                  {isEmailed
                    ? "Scanner link sent via email!"
                    : "New scanner link ready!"}
                </p>
                {!isEmailed && (
                  <>
                    <p className="text-green-300 text-xs font-mono break-all mb-3">
                      {newLink}
                    </p>
                    <button
                ...
                {isEmailed && (
                  <p className="text-green-300 text-xs">
                    The link has been sent to the team member's email.
                  </p>
                )}
                <button
                  onClick={() => {
            ...
            {station.sessions.length === 0 ? (
              ...
              station.sessions.map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className={`bg-gray-800 rounded-xl px-3 py-2.5 ${!s.isActive || expired ? "opacity-50" : ""}`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-gray-600 text-xs mt-0.5 flex items-center gap-2">
                          ...
                          {s.lastUsed && (
                            <span>· Last used {relTime(s.lastUsed)}</span>
                          )}
                        </p>
                      </div>
                    ...
                    {showUrls[s.id] && !s.isEmailed && (
                      <div className="mt-2 pt-2 border-t border-gray-700">
                        <div className="flex items-center gap-2">
                          <p className="text-gray-300 text-xs font-mono truncate flex-1 bg-gray-900 px-2 py-1 rounded">
                            {sessionUrl}
                          </p>
                          <button
...
export function ScanPanel({ event }: { event: ManagedEvent }) {
  ...
  const totalScanners = stations.reduce(
    (s, st) =>
      s +
      st.sessions.filter(
        (ss) => ss.isActive && new Date(ss.expiresAt) > new Date(),
      ).length,
    0,
  );
    ...
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div
            key={label}
            className="bg-gray-700/40 border border-gray-700 rounded-xl p-3 text-center"
          >
      ...
      <div className="bg-gray-700/30 border border-gray-700 rounded-xl px-4 py-3 text-gray-500 text-xs leading-relaxed">
        ...
        <p>
          • Create stations in order (e.g. Gate 1 → Security → Final Check).
        </p>
        <p>
          • Mark the last station as "Final" using the shield icon — it marks
          tickets as admitted.
        </p>
        <p>
          • Generate a scanner link per station and share with your team. No
          login needed.
        </p>
        <p>
          • Tickets must pass stations in sequence. Use "Logs" to see every scan
          attempt.
        </p>
      </div>
      ...
      {showAdd && (
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-3 text-gray-500 hover:text-gray-300 transition text-sm"
          >
            Cancel
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-800/40 border border-gray-700 rounded-2xl">
          ...
          <p className="text-gray-500 font-semibold">
            No scanning stations yet
          </p>
          <p className="text-gray-600 text-sm mt-1">
            Add your first station to start scanning tickets.
          </p>
        </div>
      ) : (
