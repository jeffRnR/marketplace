...
export function SharePanel({ shortCode, compact = false }: SharePanelProps) {
  ...
  const shortLink = `${base}/e/${shortCode}`;
    ...
    return (
      <div className="flex items-center gap-1.5">
        {/* <button
          ...
          className={`flex items-center justify-center gap-1.5 text-sm font-medium rounded-xl px-3 py-2.5 border transition duration-300 ${copied
        ...
        {canNativeShare && (
          <button
            onClick={nativeShare}
            title="Share event"
            className="flex-1 flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-lg py-2.5 transition px-3"
          >
    ...
    <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex flex-col gap-4">
      <div>
        ...
        <p className="text-xs text-gray-500">
          Share this one link everywhere: WhatsApp, Instagram, TikTok,
          printed flyers. We automatically detect where your visitors came
          from.
        </p>
      </div>

      <div className="flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2">
        <span className="text-sm text-purple-300 font-mono flex-1 truncate">
          {shortLink}
        </span>
      ...
      <p className="text-[10px] text-gray-600 text-center">
        Analytics will show which platform each visitor came from automatically.
      </p>
    </div>
  );
}